...
option_specification = matrix(c(
  'input.data','i',2,'character',
   ...
   'taxrank','x',2,'character',
    ...
    'g.column','g1',2,'numeric',
     'g.group','g2',2,'character',
...
obs.table<-read.table(options$obs.data,header=T,sep="\t",stringsAsFactors = F,comment.char="")
...
### make sure the sample ID is in the first column of metadata
#group1<-metadata.table[which(metadata.table$Tissue %in% group.df$group[1]),]$SampleID
#group2<-metadata.table[which(metadata.table$Tissue %in% group.df$group[2]),]$SampleID
...
if(!options$norm){
### raw count table
    suppressMessages(raw.count.deseq.obj<-DESeqDataSetFromMatrix(countData = input.table,colData=metadata.table, as.formula(paste('~',column.name,sep=""))))
